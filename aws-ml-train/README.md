...
```bash
aws-ml-train$ sam build --use-container
```
...
```bash
aws-ml-train$ sam logs -n StockCheckerFunction --stack-name aws-ml-train --tail
```
...
```bash
aws-ml-train$ pip install -r tests/requirements.txt --user
# unit test
aws-ml-train$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
aws-ml-train$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name aws-ml-train
```
